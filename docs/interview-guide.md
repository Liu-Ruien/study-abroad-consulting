...
> 适合口头回答，每题控制在 30–60 秒。日文以自然口语为主。

---

### Q1：为什么选择做出国咨询信息整理平台？

**中文：** 留学信息分散，又是真实场景，适合练产品拆解。我想做一个能演示页面、规则和边界的完整作品集，而不是教程 clone。

**日本語：** 留学情報は分散していて、プロダクト分解の題材に向いています。チュートリアルの写経ではなく、ページ・ルール・境界まで説明できるポートフォリオにしたかったです。

---

### Q2：为什么不直接做一个商业咨询网站？

**中文：** 目标是求职展示，不是运营。商业站需要客服、合规、内容和后端，会偏离我想证明的前端与产品设计能力。

**日本語：** 目的は就職ポートフォリオであり、商用運用ではありません。商用だとサポートやコンプライアンスが必要になり、示したい実装力から外れるためです。

---

### Q3：为什么没有接入真实 AI API？

**中文：** 当前要验证的是问答结构和边界，不是模型能力。真实 API 有成本、密钥和合规问题，mock 更适合作品集阶段。

**日本語：** 今はモデル性能より、問答構造と境界の検証が目的です。本番 API はコストや鍵管理が必要なので、ポートフォリオ段階では mock が適切です。

---

### Q4：为什么没有使用数据库？

**中文：** 没有用户系统和历史需求，文章和规则用 TS 文件就够。部署简单，代码也更容易在面试里讲清楚。

**日本語：** ユーザー管理や履歴保存の要件がないため、記事とルールは TS ファイルで十分です。デプロイも説明もシンプルに保てます。

---

### Q5：`/plan` 的规则逻辑是怎么设计的？

**中文：** 输入映射到候选路线，scoring 算匹配分，insight 生成可读理由。逻辑在 lib/plan，页面只负责表单和展示，方便单独讲清流程。

**日本語：** 入力をルート候補にマッチさせ、スコアリングと説明文を出します。ロジックは `lib/plan` に分離し、面接で流れを説明しやすくしています。

---

### Q6：多语言是如何实现的？

**中文：** UI 文案在 lib/i18n，Case Study 独立 i18n 文件，文章用 getLocalizedArticle。语言存 localStorage，演示时切换方便。

**日本語：** UI は `lib/i18n`、Case Study は専用ファイル、記事は取得関数で切替。言語は localStorage に保存し、デモで切り替えやすくしています。

---

### Q7：这个项目最能体现你的什么能力？

**中文：** 独立交付：从需求到部署；能把模糊问题拆成页面和 lib 逻辑；对 AI 功能有边界意识；文档和版本记录完整，适合讲解。

**日本語：** 要件からデプロイまで一人で進められる点、プロダクト分解、AI の境界設計、ドキュメント整備を示せる点です。

---

### Q8：如果继续迭代，你会优先做什么？

**中文：** 测试、错误状态、更多真实内容，以及 /plan 和 /ai 联动。接真实 AI 前先写免责声明和安全策略，不会先堆登录和数据库。

**日本語：** テスト、エラー処理、コンテンツ追加、`/plan` と `/ai` の連携です。本番 AI 前に免責と安全設計。ログインや DB は後回しです。

---

### Q9：这个项目和普通教程项目有什么区别？

**中文：** 教程多是跟做 UI。这里有自选题、规则引擎、mock AI、三语言 Case Study 和完整 docs，能讲为什么这样设计。

**日本語：** チュートリアルは UI 写経が中心ですが、本プロジェクトは題材選定、ルールエンジン、mock AI、Case Study、docs まで自分で設計しています。

---

### Q10：你在这个项目中遇到的最大问题是什么？

**中文：** 范围控制。功能一多就容易像商业产品。我通过 project-status 和明确边界（不接 API、不接 DB）强制每版只做一件事，比如先稳定 /plan 再做 /ai 和 case-study。

**日本語：** スコープ管理が最大の課題でした。機能を増やしすぎないよう、版ごとに目的を決め、API や DB を入れない境界を守りながら段階的に進めました。
...
*文档版本：v0.19.0 · 与 project-status.md 同步维护*
