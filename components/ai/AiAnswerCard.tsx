...
export default function AiAnswerCard({ answer }: AiAnswerCardProps) {
  ...
  return (
    <>
      <div
        aria-live="polite"
        className={`pointer-events-none fixed left-1/2 top-20 z-[70] max-w-[calc(100vw-2rem)] -translate-x-1/2 rounded-full bg-white/82 px-5 py-3 text-sm font-medium text-slate-900 shadow-[0_18px_45px_rgba(15,23,42,0.14)] ring-1 ring-white/80 backdrop-blur-xl transition-all duration-300 sm:top-24 ${
          isCopyToastVisible
          ...
          <div className="relative">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between sm:gap-5">
              <div className="min-w-0 flex-1">
                ...
                <h2 className="mt-3 text-3xl font-medium tracking-[-0.04em] text-slate-950 sm:pr-2 sm:text-5xl">
                  {answer.title}
                </h2>
              </div>
              ...
              <button
                ...
                className="inline-flex w-fit shrink-0 self-start rounded-full bg-white/42 px-4 py-2 text-sm font-medium text-slate-700 ring-1 ring-white/70 backdrop-blur-xl transition hover:bg-white/68 hover:text-slate-950"
              >
                复制回答
              </button>
            </div>
