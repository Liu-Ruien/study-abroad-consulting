...
export default function SummaryItem({
  ...
  return (
    <div
      className={`rounded-[22px] text-sm leading-6 text-slate-700 ring-1 transition-all duration-200 ${isEditing
        ? "bg-white ring-slate-300 shadow-sm"
        : "bg-white/75 ring-slate-200/80 hover:-translate-y-[1px] hover:bg-white hover:shadow-sm"
        }`}
    >
      <button
        type="button"
      ...
      >
        <span className={isEditing ? "shrink-0 text-slate-900" : "shrink-0 text-slate-400"}>
          {label}
        </span>
