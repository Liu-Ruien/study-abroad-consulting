...
- 当前版本：v0.8.1
- 当前阶段：第四阶段进行中，已优化 `/ai` AI 问答 MVP 体验
...
已完成 `/ai` 本地模拟问答 MVP，并在 v0.8.1 优化交互体验、空输入校验、快捷问题选中态和本地模拟回答结构。

---

## 3. v0.8.1 已优化内容

- 增加 loading 状态：提交问题后按钮显示“正在整理回答...”，回答区域显示轻量整理中提示。
- 优化快捷问题交互：点击快捷问题会自动填入输入框并直接生成回答。
- 增加快捷问题选中态：当前点击的问题会有轻微高亮。
- 优化空输入校验：空问题不会生成回答，输入框附近显示“请先输入你想了解的问题”，用户开始输入后自动消失。
- 优化本地模拟回答结构：每类回答统一为 `title`、`summary`、`suggestions`、`risks`、`nextSteps`、`disclaimer`。
- 明确回答优先级：签证 / 移民 / 永住优先，其次预算、求职、日本、语言和通用规划。
- 加强安全边界：页面和每条回答都说明不提供签证、移民、录取或就业承诺。
- 优化移动端体验：输入框高度更克制，快捷问题自然换行，回答卡片避免横向溢出。
